...
class JobConf(object):
    ...
    def __init__(self, job):
        ...
        while True:
            ...
            # see if there is a moa job & config file -
    ...
    def render(self, force=False, showPrivate=True):
        ...
        while toExpand:
            ...
            try:
                ...
                # not (yet?) possible
                if len(toExpand) == 0:
                    l.debug("unsolvable configuration " +
                            "- cannot expand '%s'" % key)
                    rv[key] = self[key]
                    break
                toExpand.append(key)
    ...
    def load(self, confFile, delta=None):
        ...
        if loadingRecursive:
            for k, v in list(y.items()):
                ...
                isFileSet = k in self.job.template.filesets

                if not (parType == 'file' or
                        parType == 'directory' or
                        isFileSet):
                    continue
                ...
                moa.ui.warn("Recursive definition of relative " +
    ...
    def has_key(self, key):
        ...
        if key in c:
            return True
        if  key in self.job.template.parameters:
            return True
        return False
    ...
    def __getitem__(self, key):
        ...
        if key in c:
            v = c[key]
        elif (key in self.job.template.parameters and
              'default' in self.job.template.parameters[key]):
            v = self.job.template.parameters[key].default

        if (key in self.job.template.parameters and
                'callback' in self.job.template.parameters[key]):
